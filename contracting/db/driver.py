...
class FSDriver:
    ...
    def set(self, key, value):
        ...
        if currency_exists and filename == "currency" and h5.get_value(self.__filename_to_path("currency"), "__code__") is None:
            print(f"__code__ variable does not exist before this update! KEY={key} VALUE={value}")

        h5.set_value(self.__filename_to_path(filename), variable, value)

        if currency_exists and filename == "currency" and h5.get_value(self.__filename_to_path("currency"), "__code__") is None:
            print(f"__code__ variable does not exist after this update! KEY={key} VALUE={value}")
...
class CacheDriver:
    def __init__(self, driver=None):
        ...
        self.cache = {}                      # L1 cache
        self.driver = driver or Driver()   # L0 cache
