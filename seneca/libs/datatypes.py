...
class HMap(RObject):
    ...
    def set(self, key, value):
        ...
        if type(key) in complex_types:
            key = key.rep()
        return self.driver.set('{}:{}{}{}'.format(self.contract_id, self.prefix, self.delimiter, key), v)

    def get(self, key):
        if type(key) in complex_types:
            key = key.rep()

        g = self.driver.get('{}:{}{}{}'.format(self.contract_id, self.prefix, self.delimiter, key))
