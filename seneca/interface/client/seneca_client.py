# TODO need to clean up these and change the paths
...
class SenecaClient:
    ...
    def __init__(self, mock=False):
        ...
        if not mock:
            # TODO this code will change to establishing connection to seneca server
    ...
    def _run_contract(self, contract: ContractTransaction, rerun: bool = False):
        ...
        # TODO we can bring most of run_contract code into this class itself
        #      it should run all python code locally and send only db operations to server
        #      every db-operation should accompany this info:
        #      sb-index, txn # (just index to input queue is sufficient), db-namespace, db-operation(s), assertions on db-state
        ...
        if not res:
            ...
            if rerun:
                ...
                # TODO status and state have to be filled in separately when assertion check is complete - perhaps through futures? or a direct communication from server for all contracts when check done?
                self.queue.append(TransactionData.create(contract_tx=contract, status='SUCCESS', state='over9000'))
