...
from contracting.db.driver import Driver, InMemDriver, FSDriver, LMDBDriver, AsyncDriver
import random
import asyncio
...
class TestAsyncDriver(TestCase):
    ...
    def setUp(self):
        ...
        self.loop.run_until_complete(self.d.flush())
    ...
    def test_get_set(self):
        a = 'a'
        self.loop.run_until_complete(self.d.set('b', a))

        b = self.loop.run_until_complete(self.d.get('b'))
        self.assertEqual(a, b)
    ...
    def test_delete(self):
        ...
        self.loop.run_until_complete(self.d.delete('b'))
    ...
    def test_iter(self):
        ...
        for k in keys:
            self.loop.run_until_complete(self.d.set(k, k))

        p1 = []

        keys = self.loop.run_until_complete(self.d.iter(prefix='b'))
        for k in keys:
            p1.append(k)
        ...
        keys = self.loop.run_until_complete(self.d.iter(prefix='x'))
        for k in keys:
            p2.append(k)
    ...
    def test_set_object_returns_properly(self):
        ...
        self.loop.run_until_complete(self.d.set('thing', thing))

        t = self.loop.run_until_complete(self.d.get('thing'))
        self.assertDictEqual(thing, t)

    def test_set_none_deletes(self):
        t = 123

        self.loop.run_until_complete(self.d.set('t', t))

        self.assertEqual(self.loop.run_until_complete(self.d.get('t')), 123)

        self.loop.run_until_complete(self.d.set('t', None))

        self.assertEqual(self.loop.run_until_complete(self.d.get('t')), None)
    ...
    def test_delete_sets_to_none(self):
        ...
        self.loop.run_until_complete(self.d.delete('t'))
    ...
    def test_iter_with_length_returns_list_of_size_l(self):
        ...
        keys = self.loop.run_until_complete(self.d.iter(prefix='b', length=3))
        for k in keys:
            p1.append(k)
        ...
        keys = self.loop.run_until_complete(self.d.iter(prefix='x', length=5))
        for k in keys:
            p2.append(k)
    ...
    def test_keys_returns_all_keys(self):
        ...
        got_keys = self.loop.run_until_complete(self.d.keys())
