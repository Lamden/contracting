...
class TestRuntime(TestCase):
    def setUp(self):
        self.rt = runtime.RuntimeInstance()
    ...
    def test_tracer_works_roughly(self):
        stamps = 1000
        self.rt.set_up(stmps=stamps, meter=True)
        a = 5
        self.rt.tracer.stop()
        used = self.rt.tracer.get_stamp_used()
        self.rt.clean_up()
        self.assertLess(stamps - used, stamps)

    def test_tracer_bypass_records_no_stamps(self):
        stamps = 1000
        self.rt.set_up(stmps=stamps, meter=False)
        a = 5
        b = 5
        self.rt.tracer.stop()
        used = self.rt.tracer.get_stamp_used()
    ...
    def test_arbitrary_modification_of_stamps_works(self):
        ...
        self.rt.tracer.stop()
        used_1 = self.rt.tracer.get_stamp_used()
        self.rt.tracer.set_stamp(stamps - sub)
        used_2 = self.rt.tracer.get_stamp_used()
        self.rt.clean_up()
    ...
    def test_modifying_stamps_during_tracing(self):
        ...
        self.rt.tracer.stop()
        used_1 = self.rt.tracer.get_stamp_used()
        self.rt.set_up(stmps=stamps - used_1, meter=True)
        c = 5
        d = 5
        e = 5
        self.rt.clean_up()
    ...
    def test_deduct_write_adjusts_total_writes(self):
        ...
        self.assertEqual(self.rt.writes, 0)

        self.rt.deduct_write('a', 'bad')

        self.assertEqual(self.rt.writes, 4)
    ...
    def test_deduct_write_fails_if_too_many_writes(self):
        ...
        with self.assertRaises(AssertionError):
            self.rt.deduct_write('a', 'b' * 32 * 1024)
